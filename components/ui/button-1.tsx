'use client';
/*
 * The "Liquid" animated gradient, as supplied by the owner (2026-09-18) — seven
 * large radial-gradient SVGs stacked with mix-blend-difference, their gradient
 * transform drifting slowly, so the box looks like moving liquid.
 *
 * Pasted as given, with three small changes:
 *   · imported from 'framer-motion', not 'motion/react'. They are the same
 *     library (motion is framer-motion's new package name) and framer-motion 12
 *     is already installed — adding `motion` would ship the engine twice.
 *   · `let` → `const` where nothing is reassigned, and `ease` marked `as const`,
 *     so it passes this repo's lint and framer-motion 12's stricter types.
 *
 * It animates SVG attributes from JavaScript, so the stylesheet's global
 * reduced-motion rule cannot stop it: whoever mounts this must check
 * useReducedMotion() themselves (components/work/clear-filters-button.tsx does).
 * Every copy shares one gradient id, so two Liquids with DIFFERENT colours on the
 * same page would both take the first one's colours.
 */
