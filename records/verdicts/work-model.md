...
1. Vocabulary: no "ticket", "incident", "SLA", "queue", "assignment group" on
   any screen (the owner's jargon rule, extended in `scripts/jargon.ts`).
   Teams are teams; a task is a task; a work note is a team note.
