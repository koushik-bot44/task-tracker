...
| 13 | owner test (Rahul) | Family bounced the CEO back to Today: `middleware.ts` still sent everyone but a literal MANAGER away from `/routine`, and the page's own guard used the manager rule; both now admit the CEO alone; "Monitoring managers" (invite another manager) is hidden since nobody else may hold Well Being | owner round 6 | fixed |
| 14 | owner test (logo) | The development upload fallback built its URL from APP_URL, which points at production, so a picture uploaded on a laptop never loaded; the fallback now returns a relative `/api/uploads/…` path | owner round 10 | fixed |
| 15 | work model, step 1 | `scripts/flows.ts` teardown swept by the REAL CEO's id: every note, private note, meeting and invite the CEO ever made on the clone went with each run (study finding, 2026-09-09) | work model: rigs sweep only throwaway accounts | fixed |
| 16 | work model, step 1 (incident) | A drift check (`prisma migrate diff --from-migrations … --shadow-database-url <clone>`) RESET the local clone: Prisma drops and replays the shadow database. Production untouched. Rebuilt from `records/snapshots/prod-backup-2026-09-08T13-09-24-667Z` with the new `scripts/dev-restore-backup.ts` recipe; the owner's clone-only data added after 2026-09-04 (Test project, milestones, meetings) is lost | work model: clone rebuild recipe | fixed (data loss noted) |
