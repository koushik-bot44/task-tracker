...
/** Sugar over POST /transition with to = IN_PROGRESS. */
export const POST = route(async (req: Request, { params }: Params) => {
  const user = await requireUser();
  const raw = await req.text();
  const parsed = await parseBody(new Request(req.url, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...(raw ? JSON.parse(raw) : {}), to: "IN_PROGRESS" }) }), transitionSchema);
